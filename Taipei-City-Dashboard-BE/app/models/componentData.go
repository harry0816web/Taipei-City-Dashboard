...
func GetStaticChartData(query *string) (chartDataOutput any, err error) {
	rows, err := DBDashboard.Raw(*query).Rows()
	if err != nil {
		return chartDataOutput, err
	}
	defer rows.Close()
	...
	if err := json.Unmarshal(rawData, &chartDataOutput); err != nil {
		return chartDataOutput, err
	}
